...
type SimpleInterface struct {
	...
	Methods    func([]Log) ([]Log, RemoteObjectError)
}
...
func (obj *SimpleObject) Methods(val []Log) ([]Log, RemoteObjectError) {

	return val, RemoteObjectError{""}
}
...
func TestFinal_Connection(t *testing.T) {
	...
	/**
	logs := make([]Log, 0)
	logs = append(logs, Log{1, 100})
	logs = append(logs, Log{12, 100})
	lg, rm := stub.Methods(logs)
	if lg != nil || rm.Error() != "" {
